...
import { CacheService } from "./upstash";
...
export class CacheAnalytics {
  ...
  // Store metrics in Redis for persistence and dashboard access
  ...
  async getHistoricalMetrics(days: number = 7): Promise<CacheMetrics[]> {
    try {
  ...
  async warmPopularContent() {
    try {
      ...
      for (const kbId of popularKbs) {
        await this.cache.warmCache(kbId);
      }
...
export async function getCacheMetricsAPI() {
  const analytics = CacheAnalytics.getInstance();
  const metrics = analytics.getMetrics();
  const health = await analytics.getCacheHealth();
  const historical = await analytics.getHistoricalMetrics(7);
